...
def test_logreg():
    ...
    key = jr.PRNGKey(0)
    model = LogReg(key, ndim, nclasses, W_init=W_mle.T, b_init=b_mle)
    probs = np.array(model.predict(X))
    np.set_printoptions(precision=3)
    assert np.allclose(probs, true_probs, atol=1e-2)

    # Test training
    model = LogReg(key, ndim, nclasses, max_iter=200, l2reg=1e-5) 
    model.fit(X, y)
    probs = np.array(model.predict(X))
    assert np.allclose(probs, true_probs, atol=1e-2)
...
def test_objectives():
    ...
    nclasses = 4
    key = jr.PRNGKey(0)
    model = LogReg(key, ndim, nclasses) 
    params = model.params
    leaves, _ = jax.tree_util.tree_flatten(params)
    flat_params = jnp.concatenate([jnp.ravel(a) for a in leaves])
    nparams = len(flat_params)
    logits = model.model.apply(params, X)
    l2reg = 0.1
    sigma = np.sqrt(1/l2reg)
    l1 = loss_from_logits(params, l2reg, logits, y)
    l2 = loglikelihood_fn(params, model.model, X, y)
