...
class LogReg:
    ...
    def fit_minibatch(self, X, y, optimizer, batch_size=32):
        ...
        if optimizer.lower() == "polyak":
            solver = jaxopt.PolyakSGD(fun=objective, maxiter=self.max_iter)
        else:
            solver = OptaxSolver(opt=optimizer, fun=objective, maxiter=self.max_iter)
